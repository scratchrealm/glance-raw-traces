...
import { GlanceRawTracesComponent, GlanceRawTracesData, isGlanceRawTracesData } from './GlanceRawTracesComponent/GlanceRawTracesComponent';


function App() {
  let [data, setData] = useState<GlanceRawTracesData>()
  const [errorMessage, setErrorMessage] = useState<string>()
  const {width, height} = useWindowDimensions()

  useEffect(() => {
    getFigureData().then((data: any) => {
      if (!isGlanceRawTracesData(data)) {
